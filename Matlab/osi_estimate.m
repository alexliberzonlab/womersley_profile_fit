% this file adds OSI to the estimates presented in
% previous figures, 
% original file is called figure6b.m

% $$OSI = 0.5*(1-\tau_{mean}/\tau_{abs})$$
% where 
% \tau_{mean} = mean(tau)
% \tau_{abs} = mean(abs(tau))

% let's try to create a modified figure 6b
% Ratio of Max negative to Maximum positive WSS


% let's try to create a modified figure 6a
% frequency of occurance of negative WSS versus PI
...
for i = 1:N
    ...
    ratio(i) = length(find(dudy < 0 ))/length(dudy);
    ratio(i) = 0.5*(1-mean(dudy)/mean(abs(dudy)));
...
for i = 1:N
    ...
    % ratio(i+6) = length(find(dudy < 0 ))/length(dudy);
    ratio(i+6) = 0.5*(1-mean(dudy)/mean(abs(dudy)));
