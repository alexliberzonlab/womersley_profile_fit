...
clear all; close all;
clc;
load('./scaled_velocity_ex3_last.mat')
...
period=[4 4 4 2 2 2 1.2 1.2 1.2];
...
[KQ KQ0 KQmag KQphase Q Qf Qraw T j k n nf nt nt1 t1 w]=...
    FSDecomposition(qspline,nExp,T,nf,q);
...
for i = 1:length(k)
    ...
    [tmp,resnorm] = lsqcurvefit(fh,t(i),y1,u1,lb,ub,options,T,nu,a,KQ,KQ0,nf,rho);%finds tje time stamp for which the compatibilty between model and measured profiles is maximal.
    %ts(i,:) = tmp;% model time stamp for a given set 
    col=['b';'k';'r';'g';'y';'c';'m'];
    plot(y1,u1,'.','Color',sprintf('%c',col(i)));hold on;
    plot(y*8.5,fh(tmp,y,T,nu,a,KQ,KQ0,nf,rho),'--','Color',sprintf('%c',col(i)));
...
ts(nExp,i)=tmp/T;%phase t/T of the model , for which the fitting was made
end
